...
import { Save, PenTool, CheckCircle } from 'lucide-react';
...
const SmartEditor: React.FC<SmartEditorProps> = ({ initialContent, onConfirm, onCancel }) => {
  ...
  // QUAN TRỌNG: Ép dữ liệu AI vào ô soạn thảo ngay khi nhận được
  useEffect(() => {
    if (initialContent) {
    ...
    <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col h-[600px]">
      <div className="bg-white border-b border-slate-100 px-8 py-5 flex justify-between items-center">
        ...
        <div className="flex gap-4">
             ...
             <button onClick={() => onConfirm(content)} className="px-8 py-2.5 bg-indigo-600 text-white rounded-xl font-bold shadow-lg shadow-indigo-100 transition-all active:scale-95">
                <Save className="w-4 h-4 mr-2 inline" /> Xác nhận tích hợp
             </button>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-64 bg-slate-50 border-r border-slate-100 p-4 space-y-2">
            <button onClick={() => setActiveTab('objectives')} className={`w-full text-left p-4 rounded-xl font-bold ${activeTab === 'objectives' ? 'bg-white shadow text-indigo-600' : 'text-slate-50'}`}>1. Mục tiêu</button>
            <button onClick={() => setActiveTab('materials')} className={`w-full text-left p-4 rounded-xl font-bold ${activeTab === 'materials' ? 'bg-white shadow text-indigo-600' : 'text-slate-50'}`}>2. Học liệu</button>
        </div>
