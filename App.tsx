...
import { 
  ...
  Smartphone, Zap, Layers, Cpu, Phone
} from 'lucide-react';
...
const App: React.FC = () => {
  const APP_VERSION = "v2.1.0"; // Phiên bản hiện tại
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.name.endsWith('.docx')) {
      setState(prev => ({ ...prev, file, result: null, generatedContent: null, step: 'upload', logs: [`✓ Đã nhận: ${file.name}`] }));
    } else { 
  ...
  const handleAnalyze = async () => {
    ...
    if (!state.file || !state.subject || !state.grade) { alert("Anh vui lòng chọn đủ thông tin môn học và lớp!"); return; }

    setState(prev => ({ ...prev, isProcessing: true, logs: [`🚀 Hệ thống ${APP_VERSION} khởi động...`] }));

    try {
      const modelName = PEDAGOGY_MODELS[pedagogy as keyof typeof PEDAGOGY_MODELS]?.name || "Linh hoạt";
      addLog(`⚙️ Chiến lược: ${modelName}`);
      addLog("Đang xử lý dữ liệu giáo án...");
      ...
      addLog(`✓ AI đã thiết kế xong hoạt động.`);
  ...
  const handleFinalizeAndDownload = async (finalContent: GeneratedNLSContent) => {
    ...
    try {
      ...
      setState(prev => ({ 
        ...
        logs: [...prev.logs, "✨ Hoàn tất xuất file!"] 
      }));
    } catch (error) {
       addLog(`❌ Lỗi: ${error instanceof Error ? error.message : "Unknown"}`);
       setState(prev => ({ ...prev, isProcessing: false }));
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 flex flex-col items-center selection:bg-indigo-100 selection:text-indigo-900">
              ...
              <div className="flex items-center gap-3 shrink-0">
                  ...
                  <div className="flex flex-col">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] text-slate-500 font-medium uppercase tracking-wider bg-slate-100 px-1.5 py-0.5 rounded">{APP_VERSION} AI Core</span>
      ...
      <div className="w-full max-w-7xl px-4 py-8 flex flex-col gap-8">
        ...
        <div className="flex justify-center">
             <div className="flex items-center gap-4 bg-white px-6 py-2 rounded-full shadow-sm border border-slate-100">
                ...
                <div className={`flex items-center gap-2 ${state.step === 'review' ? 'text-indigo-600 font-bold' : 'text-slate-400'}`}><span className="w-5 h-5 rounded-full border-2 flex items-center justify-center text-[10px] border-current">2</span> Xử lý</div>
                <div className="w-8 h-px bg-slate-200"></div>
                <div className={`flex items-center gap-2 ${state.step === 'done' ? 'text-indigo-600 font-bold' : 'text-slate-400'}`}><span className="w-5 h-5 rounded-full border-2 flex items-center justify-center text-[10px] border-current">3</span> Hoàn tất</div>
             </div>
        </div>
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            {state.step === 'upload' && (
              <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-white overflow-hidden ring-1 ring-slate-100">
                  ...
                  <div className="p-8 space-y-8">
                      <div className="grid grid-cols-2 gap-6">
                          <div className="space-y-2">
                              ...
                              <select className="w-full p-3.5 rounded-xl border border-slate-200 bg-slate-50/50 text-sm font-semibold text-slate-700 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all appearance-none" value={state.subject} onChange={(e) => setState(prev => ({...prev, subject: e.target.value as SubjectType}))}>
                                  ...
                                  <optgroup label="Môn học Tự chọn & Năng khiếu">
                      ...
                      <div className="space-y-2">
                          <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wide flex items-center gap-1"><Layers className="w-3 h-3" /> Mô hình dạy học</label>
                          ...
                          <div className="flex flex-col items-center justify-center text-center p-4 z-10">
                              {state.file ? (
                                  <div className="flex flex-col items-center gap-2 animate-fade-in-up">
                                      ...
                                      <div><p className="font-bold text-indigo-900 text-sm">{state.file.name}</p><p className="text-xs text-indigo-500">File đã được nạp thành công</p></div>
                                  </div>
                              ) : (
                                  <div className="flex flex-col items-center gap-2 group-hover:scale-105 transition-transform duration-300">
                                      ...
                                      <div><p className="font-bold text-slate-600 text-sm">Tải lên file giáo án Word (.docx)</p><p className="text-[10px] text-slate-400 mt-1">Hỗ trợ đầy đủ bảng biểu và công thức</p></div>
                                  </div>
                              )}
                          </div>
                      ...
                      >
                        {state.isProcessing ? (<><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> AI đang tính toán...</>) : (<><Wand2 className="w-5 h-5" /> Kích hoạt AI & Tích hợp ngay</>)}
                      </button>
                  </div>
              </div>
            )}
            ...
            {state.step === 'done' && state.result && (
              <div className="bg-white rounded-3xl p-10 shadow-2xl shadow-indigo-100/50 border border-white flex flex-col items-center text-center animate-fade-in-up">
                 ...
                 <h3 className="text-2xl font-bold text-slate-800">Cập nhật thành công!</h3>
                 ...
                 <div className="flex gap-4">
                     <button onClick={() => setState(prev => ({ ...prev, step: 'upload', result: null, generatedContent: null }))} className="px-6 py-3 rounded-xl font-bold text-sm text-slate-600 hover:bg-slate-50 border border-slate-200">Làm bài tiếp</button>
          ...
          <div className="lg:col-span-4 flex flex-col gap-6 h-full">
             <div className="bg-[#1e1e2e] rounded-2xl p-5 shadow-2xl shadow-slate-400/20 flex flex-col h-[320px] border border-slate-700/50 relative overflow-hidden group">
                ...
                <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-700/50">
                    <div className="flex items-center gap-2 text-slate-300 text-xs font-bold uppercase tracking-wider font-mono"><Cpu className="w-3.5 h-3.5 text-indigo-400" /> Hệ điều hành AI</div>
             ...
             <div className="bg-white/60 backdrop-blur-sm rounded-2xl p-6 shadow-sm border border-slate-200/60 flex-1">
                ...
                <div className="space-y-4">
                    <div className="p-4 bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow group">
                      ...
                      <p className="text-[11px] text-slate-600">GV Trường THPT Lý Nhân Tông</p>
                      <p className="text-[11px] text-slate-500 flex items-center gap-1 mt-1"><Phone className="w-3 h-3" /> 097 8386 357</p>
                    </div>
                    <div className="p-4 bg-indigo-50 rounded-xl border border-indigo-100 shadow-sm">
                      <p className="text-xs font-bold text-indigo-800 mb-1">Phiên bản: {APP_VERSION}</p>
                      <p className="text-[11px] text-indigo-600 leading-relaxed italic">Ứng dụng mô hình AI Generative tiên tiến để nâng cao Năng lực Số trong giáo dục.</p>
                    </div>
                </div>
             </div>
          </div>
      ...
      <div className="w-full mt-auto py-6 text-center border-t border-slate-100 bg-white/50">
          <p className="text-[10px] text-slate-400 font-medium uppercase tracking-widest">© 2026 NLS Integrator Pro — Cung cấp giải pháp Sư phạm Số thông minh</p>
      </div>
    </div>
  );
};
