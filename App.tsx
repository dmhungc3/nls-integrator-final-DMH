...
import { 
  ...
  User, School, Phone, Activity, Terminal, Cpu, MonitorSmartphone
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [mode, setMode] = useState<IntegrationMode>('NLS'); // Mặc định là NLS
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // Truyền thêm tham số mode vào đây
  ...
  const handleFinalizeAndDownload = async (finalContent: GeneratedNLSContent) => {
    ...
    try {
      // Truyền thêm tham số mode vào hàm inject
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-6 font-sans text-slate-900 flex flex-col items-center selection:bg-indigo-100">
      ...
      <div className="relative z-10 w-full max-w-7xl mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/60 backdrop-blur-md p-4 rounded-3xl border border-white/50 shadow-sm">
        <div className="flex items-center gap-4">
           <div className="w-10 h-10 bg-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-indigo-200"><User className="w-5 h-5" /></div>
        ...
        <div className="flex items-center justify-end">
            {isKeySaved ? (
                <div className="group flex items-center gap-3 bg-white/80 p-2 pl-4 pr-2 rounded-full border border-emerald-100 shadow-sm hover:shadow-md transition-all">
                    <div className="flex items-center gap-2 text-emerald-600 font-bold text-xs"><span>API Ready</span></div>
                    <button onClick={handleEditKey} className="px-3 py-1.5 rounded-full bg-slate-100 text-slate-500 text-xs font-semibold hover:text-indigo-600 transition-colors">Thay đổi</button>
                </div>
            ) : (
                <div className="flex items-center gap-2 bg-white p-1 rounded-2xl border border-indigo-100 shadow-sm w-full md:w-80">
                    <div className="pl-3"><Key className="w-4 h-4 text-amber-500" /></div>
                    <input type="password" value={userApiKey} onChange={(e) => setUserApiKey(e.target.value)} placeholder="Nhập Gemini API Key..." className="w-full bg-transparent outline-none text-sm font-medium text-slate-700 h-9" />
                    <button onClick={saveKeyToLocal} className="px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 shadow-md transition-all">Lưu</button>
                </div>
            )}
        </div>
      </div>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-2">
                          ...
                          <div className="relative">
                            <select 
                                className="w-full p-4 rounded-2xl border border-slate-200 bg-white/50 text-sm font-medium text-slate-700 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 appearance-none"
