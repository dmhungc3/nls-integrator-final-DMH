...
const App: React.FC = () => {
  const APP_VERSION = "v4.3.0-ULTIMATE-VIEW"; 
    ...
    <div className="min-h-screen bg-slate-50 font-sans p-4 md:p-8">
      <div className="max-w-6xl mx-auto bg-white rounded-3xl shadow-sm border p-6 mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-indigo-600 rounded-2xl text-white shadow-lg"><Sparkles size={32}/></div>
          <div>
            <h1 className="text-2xl font-black text-slate-800">NLS Integrator Pro</h1>
            <p className="text-xs font-bold text-indigo-600 uppercase">{APP_VERSION}</p>
          </div>
        </div>
        <div className="flex gap-2 w-full md:w-auto">
          ...
          <button onClick={() => localStorage.setItem('gemini_api_key', userApiKey)} className="bg-slate-800 text-white px-6 py-2 rounded-xl font-bold text-sm">Lưu</button>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8">
            ...
            <div className="bg-white rounded-[2.5rem] shadow-xl p-8 md:p-12 border border-slate-100 space-y-8 animate-in fade-in duration-500">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase flex items-center gap-2"><BookOpen size={12}/> Chọn Môn học (2018)</label>
                  <select className="w-full p-4 rounded-2xl border-2 border-slate-50 bg-slate-50 font-bold text-slate-700 outline-none focus:border-indigo-500 focus:bg-white transition-all cursor-pointer" value={state.subject} onChange={e => setState(prev => ({...prev, subject: e.target.value as SubjectType}))}>
                    ...
                    <optgroup label="Bắt buộc"><option value="Toán">Toán học</option><option value="Ngữ văn">Ngữ văn</option><option value="Tiếng Anh">Tiếng Anh</option><option value="Lịch sử">Lịch sử</option></optgroup>
                    <optgroup label="Lựa chọn"><option value="Vật lý">Vật lý</option><option value="Hóa học">Hóa học</option><option value="Sinh học">Sinh học</option><option value="Tin học">Tin học</option><option value="Công nghệ">Công nghệ</option></optgroup>
                  </select>
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase flex items-center gap-2"><Layers size={12}/> Chọn Khối lớp</label>
              ...
              <label className="flex flex-col items-center justify-center w-full h-72 border-4 border-dashed border-slate-100 bg-slate-50/50 rounded-[2rem] cursor-pointer hover:bg-indigo-50 transition-all duration-300">
                <FileUp size={64} className="text-indigo-400 mb-4"/>
                <p className="text-lg font-extrabold text-slate-600">{state.file ? state.file.name : "Nạp giáo án (.docx) của thầy"}</p>
                <input type="file" accept=".docx" className="hidden" onChange={e => e.target.files && setState(prev => ({...prev, file: e.target.files![0]}))}/>
              </label>

              <button disabled={!state.file || !state.subject || !state.grade || state.isProcessing} onClick={handleAnalyze} className="w-full py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-black shadow-2xl transition-all uppercase tracking-widest active:scale-[0.98]">
                {state.isProcessing ? <RefreshCw className="animate-spin inline mr-2"/> : null} 
                BẮT ĐẦU TÍCH HỢP NLS
              </button>
            </div>
          )}
          ...
          {state.step === 'review' && state.generatedContent && (
            <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden animate-in zoom-in-95 duration-500">
              <div className="p-6 border-b bg-slate-50 flex justify-between items-center">
                <h3 className="font-black text-slate-800 text-lg uppercase tracking-tight">Xem trước NLS chèn vào Word</h3>
                ...
                {activeTab === 'activities' ? (
                  <div className="space-y-6">
                    {state.generatedContent.activities_integration.map((act, i) => (
                      <div key={i} className="bg-blue-50/30 p-6 rounded-3xl border border-blue-100">
                        <div className="flex items-center gap-2 mb-3 text-blue-700 font-black text-xs uppercase border-b border-blue-100 pb-2"><Zap size={16}/> Chèn vào: {act.anchor_text}</div>
                        <p className="text-sm text-blue-900 leading-relaxed font-medium">👉 {act.content}</p>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="text-sm text-emerald-900 font-bold whitespace-pre-wrap leading-relaxed italic bg-emerald-50/30 p-8 rounded-3xl border border-emerald-100 shadow-inner">
          ...
          {state.step === 'done' && (
            <div className="bg-white rounded-[2.5rem] p-16 text-center shadow-2xl border border-emerald-100">
              ...
              <h2 className="text-3xl font-black text-slate-800 mb-4 tracking-tight">Hoàn tất rực rỡ thầy Hùng ơi!</h2>
              <button onClick={() => window.location.reload()} className="bg-slate-800 text-white px-12 py-4 rounded-2xl font-bold shadow-xl active:scale-95 transition-all">Làm bài khác</button>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white p-6 rounded-[2rem] shadow-lg border">
            ...
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 bg-indigo-600 text-white rounded-2xl flex items-center justify-center font-black text-xl shadow-lg shadow-indigo-100">MH</div>
              <div><p className="font-black text-slate-800 text-lg leading-tight">Đặng Mạnh Hùng</p><p className="text-xs text-slate-500 font-bold uppercase italic">THPT Lý Nhân Tông</p></div>
            </div>
          </div>
