...
const App: React.FC = () => {
  const APP_VERSION = "v3.3.7-FINAL-UI"; 
  ...
  const autoDetectInfo = (fileName: string) => {
    ...
    // Ép kiểu an toàn để tránh lỗi đỏ
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file?.name.endsWith('.docx')) {
      ...
      setState(prev => ({ 
        ...
        logs: [
            `✅ Đã nhận: ${file.name}`, 
            finalSubject ? `✅ Môn: ${finalSubject}` : "❓ Thầy hãy chọn môn", 
            finalGrade ? `✅ Lớp: ${finalGrade}` : "❓ Thầy hãy chọn lớp"
        ].filter(Boolean)
      }));
    }
  };
  ...
  const handleAnalyze = async () => {
    ...
    setState(prev => ({ 
      ...
      logs: [...prev.logs.filter(l => !l.includes("❓")), `✅ Cấu hình: ${state.subject} - ${state.grade}`, `⚡ Khởi động Core ${APP_VERSION}...`, `🤖 Đang thiết kế NLS chi tiết...`] 
    }));
    ...
    try {
      ...
      addLog(`✨ Hoàn tất tích hợp NLS & Prompt.`);
      ...
      <div className="w-full max-w-7xl px-4 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          {state.step === 'review' && state.generatedContent && (
            <div className="bg-white rounded-3xl shadow-xl border overflow-hidden">
              <div className="p-4 border-b flex justify-between bg-slate-50"><h3 className="font-bold text-indigo-900">Kết quả tích hợp</h3><button onClick={handleFinalizeAndDownload} className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-xs font-bold">Tải về</button></div>
              <div className="grid grid-cols-4 border-b text-xs font-bold text-slate-500">
                {['objectives', 'materials', 'activities', 'matrix'].map(t => (
                  <button key={t} onClick={() => setActiveTab(t as any)} className={`py-3 ${activeTab === t ? 'text-indigo-600 border-b-2 border-indigo-600' : ''}`}>{t === 'objectives' ? 'Mục tiêu' : t === 'materials' ? 'Học liệu' : t === 'activities' ? 'Hoạt động' : 'Ma trận'}</button>
                ))}
              </div>
                  ...
                  <div className="space-y-4">
                    {state.generatedContent.activities_integration?.map((act, i) => (
                      <div key={i} className="p-4 bg-slate-50 rounded-xl border">
                        <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest mb-2">Mốc: {act.anchor_text}</p>
                        <p className="text-sm text-slate-700 font-medium">⚡ {act.content.split('[Câu lệnh mẫu]:')[0]}</p>
                        {act.content.includes('[Câu lệnh mẫu]:') && (
                          <div className="mt-2 p-2 bg-indigo-50 border-l-4 border-indigo-400 rounded"><p className="text-xs italic text-indigo-900">"{act.content.split('[Câu lệnh mẫu]:')[1].trim()}"</p></div>
                        )}
                      </div>
                    ))}
                  </div>
                ) : (
                  <pre className="whitespace-pre-wrap text-sm font-sans text-slate-700">{activeTab === 'objectives' ? state.generatedContent.objectives_addition : activeTab === 'materials' ? state.generatedContent.materials_addition : state.generatedContent.appendix_table}</pre>
                )}
              </div>
            </div>
          )}
          ...
          {state.step === 'done' && (
            <div className="bg-white rounded-3xl p-10 shadow-2xl text-center border border-emerald-100">
              ...
              <button onClick={() => window.location.reload()} className="mt-4 text-indigo-600 font-bold underline">Làm bài khác</button>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-4">
          <div className="sticky top-24">
            {/* THÔNG TIN TÁC GIẢ - ĐƯA LÊN TRÊN CÙNG ĐỂ KHÔNG BỊ CHE */}
            ...
            {/* TERMINAL - GIẢM CHIỀU CAO XUỐNG 380PX */}
