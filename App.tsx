...
const App: React.FC = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.name.endsWith('.docx')) {
      setState(prev => ({ ...prev, file, result: null, generatedContent: null, step: 'upload', logs: [`✓ Đã nhận file: ${file.name}`] }));
    } else { 
      alert("Hệ thống chỉ hỗ trợ định dạng .docx!"); 
    }
  };
  ...
  const handleAnalyze = async () => {
    if (!userApiKey.trim()) { alert("Vui lòng nhập Gemini API Key!"); return; }
    if (!state.file || !state.subject || !state.grade) { alert("Anh vui lòng chọn đủ Môn học và Khối lớp!"); return; }

    setState(prev => ({ ...prev, isProcessing: true, logs: [`🚀 Khởi động nhân AI: ${mode === 'NAI' ? 'Năng lực AI' : 'Năng lực Số'}...`] }));

    try {
      const currentModel = PEDAGOGY_MODELS[pedagogy as keyof typeof PEDAGOGY_MODELS]?.name || "Linh hoạt";
      addLog(`⚙️ Cấu hình mô hình: ${currentModel}`);
      addLog("Đang trích xuất nội dung giáo án...");
      ...
      addLog(`✓ AI đã hoàn thành thiết kế hoạt động.`);
      setState(prev => ({ ...prev, isProcessing: false, generatedContent, step: 'review' }));
    } catch (error) {
      addLog(`❌ Lỗi hệ thống: ${error instanceof Error ? error.message : "Xung đột dữ liệu"}`);
  ...
  const handleFinalizeAndDownload = async (finalContent: GeneratedNLSContent) => {
    ...
    setState(prev => ({ ...prev, isProcessing: true, logs: [...prev.logs, "Đang đóng gói file giáo án mới..."] }));
    try {
      ...
      setState(prev => ({ 
        ...
        isProcessing: false, 
        step: 'done', 
        result: { fileName: `Tích-hợp-${mode}-${state.file?.name}`, blob: newBlob }, 
        logs: [...prev.logs, "✨ Xuất file thành công!"] 
      }));
    } catch (error) {
       addLog(`❌ Lỗi đóng gói: ${error instanceof Error ? error.message : "Unknown"}`);
       setState(prev => ({ ...prev, isProcessing: false }));
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 flex flex-col items-center selection:bg-indigo-100 selection:text-indigo-900">
              ...
              <div className="flex items-center gap-3 shrink-0">
                  ...
                  <div className="flex flex-col">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] text-slate-500 font-medium uppercase tracking-wider bg-slate-100 px-1.5 py-0.5 rounded">v2.0 AI Core</span>
              ...
              <div className="flex items-center justify-end shrink-0">
                  {isKeySaved ? (
                      <div className="flex items-center gap-2 bg-emerald-50/80 px-4 py-1.5 rounded-full border border-emerald-100 shadow-sm">
                          ...
                          <button onClick={handleEditKey} className="ml-2 text-[10px] text-slate-400 hover:text-indigo-600 underline">Đổi Key</button>
                      </div>
                  ) : (
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            {state.step === 'upload' && (
              <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-white overflow-hidden ring-1 ring-slate-100 animate-fade-in-up">
                  ...
                  <div className="p-8 space-y-8">
                      {/* CHỌN MÔN VÀ KHỐI LỚP PHIÊN BẢN CHUẨN NHẤT */}
                          ...
                          <div className="space-y-2">
                              ...
                              <select className="w-full p-3.5 rounded-xl border border-slate-200 bg-slate-50/50 text-sm font-semibold text-slate-700 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all appearance-none" value={state.subject} onChange={(e) => setState(prev => ({...prev, subject: e.target.value as SubjectType}))}>
                                  ...
                                  <optgroup label="Cơ bản">
                                    <option value="Toán">Toán học</option><option value="Ngữ văn">Ngữ văn</option><option value="Tiếng Anh">Tiếng Anh</option>
                                  </optgroup>
                                  <optgroup label="Tự nhiên & Công nghệ">
                                    <option value="Vật lí">Vật lí</option><option value="Hóa học">Hóa học</option><option value="Sinh học">Sinh học</option><option value="Tin học">Tin học</option><option value="Công nghệ">Công nghệ</option>
                                  </optgroup>
                                  <optgroup label="Xã hội & Đặc thù">
                                    <option value="Lịch sử">Lịch sử</option><option value="Địa lí">Địa lí</option><option value="Giáo dục thể chất">Giáo dục thể chất</option><option value="Âm nhạc">Âm nhạc</option><option value="Mỹ thuật">Mỹ thuật</option>
                                  </optgroup>
                              </select>
                          </div>
                          ...
                          <div className="flex flex-col items-center justify-center text-center p-4 z-10">
                              {state.file ? (
                                  <div className="flex flex-col items-center gap-2 animate-fade-in-up">
                                      ...
                                      <div><p className="font-bold text-indigo-900 text-sm">{state.file.name}</p><p className="text-xs text-indigo-500">File đã sẵn sàng để tích hợp</p></div>
                                  </div>
                              ) : (
                                  <div className="flex flex-col items-center gap-2 group-hover:scale-105 transition-transform duration-300">
                                      ...
                                      <div><p className="font-bold text-slate-600 text-sm">Nhấn để tải file giáo án (.docx)</p><p className="text-[10px] text-slate-400 mt-1">Đảm bảo giáo án soạn thảo theo mẫu chuẩn</p></div>
                                  </div>
                              )}
                          </div>
                      ...
                      >
                        {state.isProcessing ? (<><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> AI đang thiết kế giải pháp...</>) : (<><Wand2 className="w-5 h-5" /> Kích hoạt AI & Tích hợp ngay</>)}
                      </button>
                  </div>
              </div>
            )}
            ...
            {state.step === 'done' && state.result && (
              <div className="bg-white rounded-3xl p-10 shadow-2xl shadow-indigo-100/50 border border-white flex flex-col items-center text-center animate-fade-in-up">
                 ...
                 <h3 className="text-2xl font-bold text-slate-800">Hoàn tất nâng cấp giáo án!</h3>
                 <p className="text-slate-500 mt-2 mb-8 max-w-md">Giáo án của anh đã được tích hợp các hoạt động {mode === 'NAI' ? 'Năng lực AI' : 'Năng lực Số'} tiên tiến.</p>
          ...
          <div className="lg:col-span-4 flex flex-col gap-6 h-full">
             <div className="bg-[#1e1e2e] rounded-2xl p-5 shadow-2xl shadow-slate-400/20 flex flex-col h-[320px] border border-slate-700/50 relative overflow-hidden group">
                ...
                <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-700/50">
                    <div className="flex items-center gap-2 text-slate-300 text-xs font-bold uppercase tracking-wider font-mono"><Cpu className="w-3.5 h-3.5 text-indigo-400" /> Trạng thái AI Core</div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 font-mono text-[11px] leading-relaxed">
                   {state.logs.length === 0 && <span className="text-slate-600 italic">&gt;&gt; Đang chờ lệnh...</span>}
             ...
             <div className="bg-white/60 backdrop-blur-sm rounded-2xl p-6 shadow-sm border border-slate-200/60 flex-1">
                <h4 className="font-bold text-sm text-slate-800 mb-4 flex items-center gap-2 uppercase tracking-wide"><GraduationCap className="w-4 h-4 text-indigo-500" /> Góc Sư phạm Số</h4>
                <div className="space-y-4">
                    <div className="p-4 bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow group">
                      <p className="text-xs font-bold text-indigo-700 mb-1 flex items-center gap-2"><Smartphone className="w-3 h-3" /> Năng lực Số</p>
                      <p className="text-[11px] text-slate-500 leading-relaxed group-hover:text-slate-700">Tự động đề xuất phần mềm chuyên dụng: GeoGebra, Padlet, Canva phù hợp từng môn học.</p>
                    </div>
                    <div className="p-4 bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow group">
                      <p className="text-xs font-bold text-rose-700 mb-1 flex items-center gap-2"><Zap className="w-3 h-3" /> Năng lực AI</p>
                      <p className="text-[11px] text-slate-500 leading-relaxed group-hover:text-slate-700">Tích hợp AI làm trợ lý đồng hành, giúp học sinh phát triển tư duy phản biện và sáng tạo nội dung số.</p>
                    </div>
                </div>
             </div>
          </div>
      ...
      <div className="w-full mt-auto py-6 text-center border-t border-slate-100 bg-white/50">
          <p className="text-[10px] text-slate-400 font-medium uppercase tracking-widest">© 2026 NLS Integrator Pro — Phát triển bởi Đặng Mạnh Hùng — Lý Nhân Tông</p>
      </div>
    </div>
  );
};
