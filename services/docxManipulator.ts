...
const App: React.FC = () => {
  ...
  const handleFinalizeAndDownload = async (finalContent: GeneratedNLSContent) => {
    ...
    try {
      // Gọi hàm inject với tham số mode
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-6 font-sans text-slate-900 flex flex-col items-center pb-32">
      ...
      {/* --- KHỐI STICKY HEADER (Tác giả + Tên sản phẩm) --- */}
      <div className="sticky top-0 z-50 w-full max-w-7xl transition-all">
          {/* Nền mờ cho cả khối header để nội dung bên dưới trượt qua đẹp mắt */}
          ...
          <div className="pt-4 pb-4 px-2 flex flex-col gap-4">
              ...
              {/* Hàng 2: Tên sản phẩm (Được ghim cùng) */}
      ...
      {/* --- NỘI DUNG CHÍNH (Sẽ cuộn bên dưới Header) --- */}
            ...
            {state.step === 'upload' && (
              <div className="bg-white/80 backdrop-blur-xl rounded-[2rem] p-8 shadow-xl border border-white animate-fade-in">
                  ...
                  <div className="bg-slate-100 p-1 rounded-2xl flex relative mb-6">
                      ...
                      <button onClick={() => setMode('NLS')} className={`relative z-10 flex-1 py-3 text-sm font-bold flex items-center justify-center gap-2 transition-colors ${mode === 'NLS' ? 'text-indigo-600' : 'text-slate-500'}`}><MonitorSmartphone className="w-4 h-4" /> Năng lực Số</button>
                      <button onClick={() => setMode('NAI')} className={`relative z-10 flex-1 py-3 text-sm font-bold flex items-center justify-center gap-2 transition-colors ${mode === 'NAI' ? 'text-rose-600' : 'text-slate-500'}`}><Cpu className="w-4 h-4" /> Năng lực AI</button>
                  </div>
